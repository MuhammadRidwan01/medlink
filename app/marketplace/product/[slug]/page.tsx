...
export default function ProductDetailPage({ params }: ProductDetailPageProps) {
  const product = MOCK_PRODUCTS.find((item) => item.slug === params.slug);
  ...
  const relatedProducts = useMemo(() => {
    ...
    return MOCK_PRODUCTS.filter((item) =>
      item.categories.some((category) => product.categories.includes(category)) && item.id !== product.id,
    ).slice(0, 4);
  }, [product]);

  if (!product) {
    return (
      <PageShell title="Produk tidak ditemukan" subtitle="Silakan kembali ke marketplace untuk melihat produk lainnya.">
        <Link
          href="/marketplace"
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        ...
        <aside className="card-surface flex h-max flex-col gap-4 p-4">
          <div>
            ...
            <p className="text-2xl font-semibold text-foreground">Rp {product.price.toLocaleString("id-ID")}</p>
          </div>
            ...
            {relatedProducts.map((related) => (
              ...
              >
                ...
                <div className="space-y-2 p-4">
                  <p className="text-sm font-semibold text-foreground line-clamp-2">{related.name}</p>
                  <p className="text-xs text-muted-foreground line-clamp-2">{related.shortDescription}</p>
                  <p className="text-sm font-semibold text-foreground">Rp {related.price.toLocaleString("id-ID")}</p>
                </div>
              </Link>
            ))}
          </div>
