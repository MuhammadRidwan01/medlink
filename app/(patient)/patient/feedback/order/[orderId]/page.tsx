...
const NEGATIVE = ["Damaged", "Late delivery", "Missing items", "Poor packaging"];

export default function OrderFeedbackPage({ params }: { params: { orderId: string } }) {
  ...
  const onSubmit = () => {
    setSubmitted(true);
    submitFeedback({ id: params.orderId, kind: "order", rating, tags, comment: comment || undefined, contactOk, testimonialOk, createdAt: new Date().toISOString() });
  };

  return (
    <PageShell title="Nilai Pesanan" subtitle="Berikan umpan balik untuk layanan apotek" className="space-y-4">
      {submitted ? (
        <ThanksCard />
      ) : (
        <div className="space-y-4">
          <section className="space-y-2">
            ...
            <RatingStars value={rating} onChange={setRating} ariaLabel="Rate your order from 1 to 5 stars" />
          </section>
          <section className="space-y-2">
